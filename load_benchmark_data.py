...
from vars import FILE_PATH, AUTH, URI, BATCH_SIZE


def create_schema(driver):
    """Creates unique index/constraint so edge MATCH lookups run in O(1) time."""
...
def load_nodes(driver, nodes):
    """Ingests all unique nodes via UNWIND batches."""
    query = "UNWIND $batch AS id MERGE (:Paper {id: id})"
    total = len(nodes)
...
def main():
    ...
    with GraphDatabase.driver(URI, auth=AUTH) as driver:
        ...
        node_count, node_time, node_rate = load_nodes(driver, nodes_list)
        edge_count, edge_time, edge_rate = load_edges(driver, FILE_PATH)
    ...
    print(f"Node Ingest Throughput     : {node_rate:,.2f} nodes/sec ({node_count:,} nodes in {node_time:.2f}s)")
    print(f"Relationship Ingest Rate   : {edge_rate:,.2f} rels/sec ({edge_count:,} edges in {edge_time:.2f}s)")
    print(f"Verified Database State    : {v_nodes:,} nodes | {v_edges:,} relationships")
    print("=" * 60)
